...
| 取込明細の編集 | STEP1直下の「取り込んだ明細を編集する」から、区分・商品名・TAIS・種目・料金区分・各月の単位数をすべて直接編集でき、合計も即再計算。載せたくない行（特価ベッド等）は🗑で削除。単位数は手入力（スピナー無し） |
| 品目の選択/入力 | 明細カードの品目は一覧から選ぶ・自由入力のどちらも可（datalist付きコンボ）。一覧に無い商品名も手入力できる |
| 保険外の除外 | 取込時に「保険外」品目は単位数一覧から自動で除外 |
| 写真の貼り付け | ドラッグ＆ドロップに加え、Windows＋Shift＋Sで切り取った画像を Ctrl＋V で貼り付け添付できる |
...
| 営業所・担当者の記憶 | 営業所は選んだ時点、担当者（自社）は入力を終えた時点（blur）でこの端末の既定として記憶（保存不要）。2回目以降の新規作成は最初から入った状態。**記憶しても案件ごとに書き換え可**で、書き換えるとその値が新しい既定になる |
| データ移行 | JSONエクスポート/インポート（写真込み）で営業所間の受け渡し |

## 営業所配布用マニュアル

`docs/福祉用具相談報告書アプリ_操作マニュアル_第1版.docx` / `.pdf`（A4・16ページ）。
配布はPDFを推奨（Wordが無い端末でも開ける）。

```bash
node _shot.cjs        # 画面キャプチャ保存サーバ(5600) → docs/img へ
cd build && node make_manual.js   # docs/img を使って docx を生成
```

スクリーンショットは、アプリを開いた状態でブラウザのコンソールから
`__shot(要素, 'ファイル名')` を呼ぶと `docs/img` に保存される（`taiyo_fukushi_debug=1` が必要）。
機能追加時は**版数を上げて全ページ配り直す**（営業所ごとに版が混ざるのを防ぐため）。
...
|---|---|
| TAISコード | 種目行の `#####-######` |
| 区分 | 「貸与」「特定福祉用具」の2種類のみ（予防貸与も貸与に集約） |
| 品目 | 種目行のTAIS以外の文字列を正規化（`予防手すり貸与` → 品目`手すり`／区分`貸与`）。特定福祉用具の品目は 入浴補助用具（シャワーチェア/浴槽手すり/浴槽台/バスボード/シャワーキャリー）・腰掛便座（ポータブルトイレ/補高便座）まで選択可 |
...
| 担当者（自社） | 発行元ブロックの `担当： 久保 匠史`。姓名の間の空白を含めて行末まで拾う |
| ケアマネ・居宅名 | 敬称「様」が無い様式にも対応。あいさつ文より上・左の宛先ブロックを、1行目＝居宅（事業所）名／人名らしい行＝ケアマネ名 として判定（例 `ケアプランサービス 紡ぐ`＝居宅、`福島 寿恵`＝ケアマネ） |
